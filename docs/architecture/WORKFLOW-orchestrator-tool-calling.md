...
| `draft_response` | `civil-adapter` | 74K 민원 답변 학습, QLoRA on AWQ base |
| 근거 보강 (follow-up) | [`legal-adapter`](https://huggingface.co/siwo/govon-legal-adapter) | 243K 법령/판례 학습, QLoRA on AWQ base |
...
| Tool Registry | `api_lookup`, `rag_search`, `draft_response`, `synthesis` 실행 |
...
- 예:
  - 초안 작성 → `[rag_search, api_lookup, draft_response(civil-adapter)]`
  - 답변 수정 → `[rag_search, api_lookup, draft_response(civil-adapter)]`
  - 근거 보강 → `[rag_search, api_lookup, synthesis(legal-adapter)]`
  - 통계 보강 → `[api_lookup]`
