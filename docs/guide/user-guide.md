...
| `public_admin_adapter` | 행정 민원 답변 초안 생성 | 예 | LoRA 모델 (74K 쌍 학습) |
| `legal_adapter` | 법률 근거 답변 초안 생성 | 예 | LoRA 모델 (270K 문서 학습) |
...
행정 민원에 대한 답변 초안을 LoRA 특화 모델로 생성한다. 74K개 민원-답변 쌍으로 학습된 어댑터를 사용한다.
...
법률 근거를 인용한 답변 초안을 LoRA 특화 모델로 생성한다. 270K개 법률 문서로 학습된 어댑터를 사용한다.
