...
async def scenario3_public_admin_lora() -> dict:
    """Scenario 3: Public Admin LoRA — domain_adapter (v2/agent/stream)."""
    t0 = time.monotonic()
    try:
        ...
        if not ok:
            return _record(
                3,
                "Public Admin LoRA (domain_adapter)",
                False,
                elapsed,
                err,
        ...
        if not text.strip():
            return _record(
                3, "Public Admin LoRA (domain_adapter)", False, elapsed, "응답 텍스트가 비어있음"
            )
        return _record(
            3,
            "Public Admin LoRA (domain_adapter)",
        ...
        return _record(
            3, "Public Admin LoRA (domain_adapter)", False, time.monotonic() - t0, str(exc)
        )


async def scenario4_legal_lora() -> dict:
    """Scenario 4: Legal LoRA — domain_adapter (v2/agent/stream).
    ...
    try:
        # 동일 세션에서 public_admin 요청 먼저 (domain_adapter는 이전 답변 컨텍스트 필요)
        ...
        if not text.strip():
            return _record(
                4, "Legal LoRA (domain_adapter)", False, elapsed, "응답 텍스트가 비어있음"
            )
        ...
        return _record(4, "Legal LoRA (domain_adapter)", True, elapsed, detail=detail)
    except Exception as exc:
        return _record(4, "Legal LoRA (domain_adapter)", False, time.monotonic() - t0, str(exc))
