...
class AgentManager:
    ...
    def _load_all_agents(self):
        ...
        if not os.path.exists(self.agents_dir):
            logger.error(f"Agents directory not found at {self.agents_dir}")
            return
                ...
                try:
                    with open(file_path, "r", encoding="utf-8") as f:
                        ...
                        if content.startswith("---"):
                            ...
                            if len(parts) >= 3:
                                # Body is the third part
                                self.personas[agent_name] = parts[2].strip()
                            else:
        ...
        logger.info(f"Loaded {len(self.personas)} agent personas: {list(self.personas.keys())}")
