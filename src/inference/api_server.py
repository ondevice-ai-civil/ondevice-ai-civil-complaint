...
async def v2_agent_stream(
    ...
    graph.astream()을 사용하여 노드별 완료 이벤트를 SSE로 전송한다.
        ...
        try:
            async for chunk in manager.graph.astream(initial_state, config, stream_mode="updates"):
                ...
                for node_name, state_delta in chunk.items():
                    ...
                    # approval_wait가 interrupt()를 호출하면 astream이 끝나기 전에
                    # 이 이벤트가 마지막으로 전송된다.
...
async def v2_agent_run(
    ...
    try:
        # ainvoke()는 async 노드와 FastAPI event loop 내에서 안전하게 동작한다.
...
async def v2_agent_approve(
    ...
    try:
        # ainvoke()로 resume — async 노드 직접 호출
