...
def build_govon_graph(
    ...
    # --- 노드 등록 (async 노드를 closure로 adapter와 session_store 주입) ---
    # Issue #495: sync _run_async() wrapper 제거 → async 노드 직접 등록.
    # FastAPI ainvoke()/astream() 경로에서 running event loop 충돌 없음.
