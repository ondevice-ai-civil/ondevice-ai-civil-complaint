...
# ---------------------------------------------------------------------------
# langgraph 환경 호환성 패치 (모듈 레벨 — fixture 불가)
#
# langgraph 1.1.5 미만 또는 혼합 설치 환경에서 ServerInfo가
# langgraph.runtime에 없을 수 있다. 이 속성이 없으면
# 하위 모듈(tool_node.py) import 시 AttributeError가 발생한다.
#
# 모듈 레벨 import(StructuredTool, MemorySaver 등)보다 먼저
# 패치해야 하므로 fixture로 격리할 수 없다.
# langgraph ≥ 1.1.5가 보장되면 이 블록을 제거할 수 있다.
# ---------------------------------------------------------------------------
import sys
    ...
    async def tool_node(state: dict) -> dict:
        ...
        async def _run_tool(tc):
            ...
            if tool is None:
                ...
                try:
                    # ainvoke를 사용하여 args_schema 검증과 콜백 경로를 포함한
                    # 실제 StructuredTool 호출 경로를 따른다.
